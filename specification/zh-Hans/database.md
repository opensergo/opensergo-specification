...
- shardingAlgorithms:
  - : # 分片算法名称 type: # 分片算法类型 props: # 分片算法属性配置

分布式序列算法配置：

- keyGenerators:
  - : # 分布式序列算法名称 type: # 分布式序列算法类型 props: # 分布式序列算法属性配置
...
- 基于列的影子算法：通过识别 SQL 中的数据，匹配路由至影子库的场景，适用于数据名单驱动的场景
- 基于 Hint 的影子算法：通过识别 SQL 中的注释，匹配路由至影子库的场景，适用于上游系统透传标识驱动的场景
